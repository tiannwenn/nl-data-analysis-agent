...
6. **并列主因建议**：原因占比接近时，业务建议须同时覆盖，禁止只挑一条；
7. **SQL 易错清单**：禁止 WHERE 中写聚合、禁止空 SQL、禁止把 `result_name` 当表、乘号 `*` 不可丢。
...
- **多会话历史**：侧边栏支持新建对话、切换/重命名/删除历史，会话持久化到 `data/chat_history.json`；
- **停止分析**：输入框 `submit_mode="stop"`；取消标志放在模块级闩锁（避免 `STOP` 态下读写 `st.session_state` 触发 `StopException`），先清原生 STOP 再落盘「分析已停止…」并 `st.rerun()`；
- **顶栏对比度**：自建深色顶栏时，强制 Deploy / 主菜单图标为浅色，避免 Light 主题下几乎不可见。
...
| 指标公式正确性（执行时）、只读安全、SQL 语法门禁、绘图几何与图号、结果表存储、停止态 UI 收口 | **确定性工程** |
| 放量「是否成立」、并列主因建议是否写全 | **规则写在提示中 + 数字来自 SQL**；最终措辞仍可能被模型写歪，故持续用规则与验收加压 |
...
- 适合观察 ReAct 过程、图表与交互；
- 侧边栏提供示例 1–5 一键填入；支持多会话历史；
- 分析中可点输入框停止键中断，界面应显示「分析已停止…」而非冻在「Agent 分析中…」。
...
|------|--------|----------|
| 示例 1 | 聚合排序 + 柱图 | 图 8-1 |
| 示例 2 | 战区/渠道术语 + Top SKU | 图 8-2 |
| 示例 3 | 退损率全量图 + 阈值下钻原因 + 并列主因建议 | 图 8-3、图 8-4 |
| 示例 4 | 季度对比 + 增长筛选 | 图 8-5、图 8-6 |
| 示例 5 | 增长且毛利下降 + 低毛利 SKU 放量归因 + 多图 | 图 8-7～图 8-9 |

### 8.4 示例 1：门店销额排序与柱图

题目：查询 2025 年上半年每家门店销售额，从高到低排序并画柱状图。

实测结论（降序）：星河路店 1,201,810.63 元 → 科技园店 1,056,452.94 元 → 湖滨店 915,688.07 元 → 天府店 854,841.01 元；图题写明「星河路店销售额最高」。

**图 8-1　示例 1：结论与图 1**

![示例1：门店销额排序与柱图](pictures/ex1_sales.png)

### 8.5 示例 2：术语映射 + Top SKU

题目：查询 2025 年上半年华东战区即时零售渠道动销最好的 3 个 SKU（销量、销额、品类）。

术语落地：`华东`→`region`，`即时零售`→`channel_code='O2O'`。Top3 为 24 寸显示器、激光打印机、商务耳机，并可展开 `top_3_skus` 表核对。

**图 8-2　示例 2：Top3 SKU 结论与数据表**

![示例2：华东即时零售 Top SKU](pictures/ex2_top_skus.png)

### 8.6 示例 3：退损率全量对比 + 高退损原因下钻

题目：各门店上半年退损情况 → 全部门店退损率对比图 → 找出超过 5% 的门店并分析主要退款原因。

要点：

- 全量图含四店；`threshold_y=5` 高亮超阈值门店（科技园 ≈8.10%，星河路 ≈5.68%）；
- 原因下钻：星河路「不符合预期」与「质量问题」金额几乎并列（约 2.74 万 / 2.73 万）；
- **建议须覆盖并列主因**（优化商品描述/预期管理，同时加强质量管控），避免只写其中一条。

**图 8-3　示例 3：分析结论与建议**

![示例3：退损结论与建议](pictures/ex3_conclusion.png)

**图 8-4　示例 3：图 1（退损率对比）与中间表**

![示例3：退损率柱图与数据表](pictures/ex3_chart.png)

### 8.7 示例 4：Q1/Q2 销额对比与增长筛选

题目：比较每家门店 2025 年 Q1/Q2 销售额，找出 Q2 比 Q1 增长超过 10% 的门店，并生成两季度对比图。

筛出星河路店（+89.64%）、科技园店（+68.68%）；图 1 为两店 Q1/Q2 并排柱图。

**图 8-5　示例 4：增长门店结论文本**

![示例4：Q1Q2增长结论](pictures/ex4_conclusion.png)

**图 8-6　示例 4：图 1（两季度销额对比）**

![示例4：Q1Q2销额对比图](pictures/ex4_chart.png)

### 8.8 示例 5：增长且毛利下降 + 低毛利 SKU 放量

题目要求：找出 2025 年 Q2 相对 Q1 销额增长超过 10% 且毛利率下降的门店，进一步分析是否存在低毛利 SKU 放量导致整体毛利率下降，并生成合适图表。

本次 Web 实测结论与库内验算一致：目标门店为星河路店、科技园店；低毛利 SKU（24 寸显示器、激光打印机）均显著放量（增长 ≥ 1 倍），可归因拖累整体毛利率。

**图 8-7　示例 5：问题与分析结论（文字部分）**

![示例5：问题与分析结论](pictures/ex5_conclusion.png)

要点摘要：

- 星河路店：销额增长 89.64%（Q1 41.49 万元 → Q2 78.69 万元），毛利率 34.85% → 20.98%；显示器约增 5.77 倍，激光打印机约增 18.2 倍；
- 科技园店：销额增长 68.68%（Q1 39.32 万元 → Q2 66.33 万元），毛利率 32.73% → 23.96%；显示器约增 3.07 倍，激光打印机约增 4.86 倍；
- 两店上述 SKU 平均毛利率均低于 8%，存在显著放量拖累。

**图 8-8　示例 5：图 1（门店级 Q1/Q2 销额对比）**

![示例5：图1门店对比](pictures/ex5_figure1.png)

**图 8-9　示例 5：图 2（低毛利 SKU「门店 - 商品」对比）与中间表**

![示例5：图2与数据表](pictures/ex5_figure2.png)

图 2 横轴为「门店 - SKU」，避免只显示店名而漏掉商品；下方可展开 `growth_stores`、`low_margin_skus` 等中间表，便于核对 SQL 产出。该截图体现了本系统「多图有序展示 + SKU 轴防呆 + 结果可追溯」的设计效果。
...
5. **回归可重复**：e2e 脚本 + 稳定限速；
6. **交互可中断**：停止键协作取消 + 历史多会话，避免长任务卡死与上下文混乱。
...
- 多会话：新建 / 切换 / 重命名 / 删除，历史落盘；
- Light 主题下顶栏 Deploy、⋮ 强制浅色，保证深色顶栏上可读。
...
- **并列主因建议**：退款原因占比接近（差距 ≤3 个百分点）时，业务建议须同时覆盖，禁止只挑一条（示例 3 星河路店「不符合预期」与「质量问题」并列即属此类）。

### 10.4 停止分析与 Streamlit 协作取消

Streamlit `chat_input(submit_mode="stop")` 会将脚本置为终态 `STOP`。此前若在 STOP 下继续读写 `st.session_state` 或 `status.update`，会立刻触发 `StopException`，导致：
...
1. 取消意图用**模块级 session 闩锁**记录，检测 STOP 时不碰 `session_state`；
2. 先把原生 `STOP` 清回 `CONTINUE`，再落盘停止回复并 `st.rerun()`；
3. Agent 侧轮询协作取消（约 0.35s），并在吞掉 `StopException` 时同步闩锁。
...
这些改进体现的能力点：**Function Calling Agent 工程化、领域知识注入、执行层校验、数据可视化一致性、可观测与可验收、前端协作取消。**
...
| 结论文案 | 仍可能写反箭头、定性漂移、建议漏写并列主因 | finish 后规则校验或「模板填数」；并列占比自动补建议 |
| 停止体验 | 依赖内部 STOP 态清零，属 Streamlit 实现细节 | 侧栏独立「停止」按钮（纯闩锁、不触发原生 STOP） |
...
4. **持续改进**：针对图号错位、SKU 漏展示、SQL 常见坑、放量结论自相矛盾、并列主因建议漏写、停止键 UI 冻住、Light 主题顶栏对比度等问题的加固，证明 Agent 系统需要「提示词 + 代码门禁 + 交互工程」多轮驱动，而不是只靠模型一次写对。
...
| `src/app.py` | Streamlit UI（多会话、停止协作取消、顶栏样式） |
| `src/chat_store.py` | 多会话历史持久化 |
| `src/llm.py` | 模型客户端与限速 |
| `scripts/e2e_examples.py` | 题目示例 1–5 批量验收 |
| `scripts/smoke_test.py` | SQL 层冒烟测试 |
| `pictures/ex1_sales.png` … `ex5_figure2.png` | Web 实测截图（示例 1–5） |
