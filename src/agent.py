...
图表与筛选分离（必须遵守）：
- 用户要求「各门店 / 每家门店…对比图」时，图表必须含**全部**相关门店；「找出超过某阈值」只用于文字名单与下钻，**禁止**把筛选子集当成对比图唯一数据源。
- 例外：问题主线是「找出满足条件的门店再下钻分析」（如增长>10%**且**毛利率下降 + 低毛利 SKU 放量），图可以只画筛出的门店。
- 典型正确流程（退损率题，示例3）：
...
- 典型正确流程（Q1/Q2 销额对比 + 找出增长>10%，示例4）：
  ...
  2) plot_chart 用该全量结果画两季度对比图（宽表 y=['q1_sales','q2_sales'] 或长表 color=quarter）；
     title 可写「…（星河路店和科技园店增长超10%）」等结论，但图上必须仍有全部门店；
