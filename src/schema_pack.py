...
- 「各门店 / 每家门店对比图」= 全部有数据的门店都必须出现在图上；title 才可写「各门店」
- 用户说「找出…门店，比较**这些门店**并画图」（如下降门店）：只画筛出店；title 写「下降门店」或店名，**禁止**写「各门店」
- 阈值筛选（如退损率>5%、销额增长>10%）在示例3/4 中只用于文字名单与原因下钻，禁止作为「各门店对比图」的唯一数据源
...
- 门店 Q1/Q2 销额对比**正确模板**（「比较每家门店两季度销售额并找出增长>10%」时仿写；先全量再文字筛选）：
  ...
  ```
  说明：示例4 用全量结果画图（x=store_name, y=['q1_sales','q2_sales']）；增长>10% 只在文字里点名。
  若题目是「找出下降门店并比较这些门店」，则对上述结果再 `WHERE growth < 0` 后画图，title 勿写「各门店」。
""".strip()
